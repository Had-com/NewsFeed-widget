...
**Investigated, not changed at the time — later resolved (see 2026-09-01/02 entry below):**
- ~~English/Latin text inside a Glamour bold headline (e.g. "ChatGPT") renders visibly lighter than the surrounding Hebrew~~ — root cause held: Dana Yad's Latin-cursive-fallback family was a thinner-stroke style than its synthesized Hebrew bold. Resolved by switching the Glamour font entirely (see below), which has native matching-weight Latin coverage and removes the fallback mechanism outright.
...
## Bug fixes and font switch (shipped 2026-09-01/02)

| # | Bug | Fix |
|---|---|---|
| 1 | Full-article text had entire runs of words glued together with no spaces (e.g. `בחר פורום---------סקופיםהזעקהפוליטיקה...`), plus dozens of stray blank-looking lines | Root cause: a `<select>` dropdown (a forum "jump to forum" picker) whose `<option>` tags `Html.fromHtml()` doesn't recognize as block-level — it deletes the tags but inserts no separator between adjacent option text, so every category name ran into the next. Fixed by stripping `<select>…</select>` wholesale (never article content anyway), plus a general defensive fix: every HTML tag is now surrounded with a guard space before parsing, so *any* tag `Html.fromHtml()` doesn't understand can no longer glue adjacent text together, regardless of which tag causes it. The "blank lines" were leftover HTML-source indentation (tabs/spaces) with no visible content — fixed by collapsing whitespace runs and trimming each line after conversion. A `<form>` strip was tried the same way for a smaller bit of leftover moderator-controls text, then reverted after verifying against a real captured page that a single `<form>` can span nearly the entire body on this site's template — stripping it risked deleting genuine article paragraphs. |
| 2 | **Real crash, found only by checking logcat directly** — `IllegalArgumentException: Column container cannot have more than 10 elements`, firing on *every* widget update for any expanded row with 2+ full-article chunks, with no visible symptom in the widget's on-screen appearance | RemoteViews caps a `Column` at 10 direct children. The expanded-row content (thumbnail header + description/buttons + however many full-article chunks, each itself 3 elements) all emitted as direct siblings into one `Column` — 2 chunks alone was already past the limit. Fixed by wrapping the whole expanded-state block, the chunk loop's total output, and each individual chunk in their own nested `Column`s, so each collapses to a single child of its parent regardless of how much is inside. Also lowered the chunk-count memory ceiling's upper bound from a permissive 40 to 10, matching this structural limit directly. This had been silently degrading that row's render since the per-chunk "Open in browser" links were added — the widget gave no visible sign anything was wrong; only logcat showed it. |

**Font switched: Dana Yad → Playpen Sans Hebrew.** The user found Playpen Sans Hebrew (TypeTogether, via Google Fonts) and asked whether it would be more memory-efficient — it isn't (bitmap memory cost is purely a function of rendered pixel dimensions, not which font draws the glyphs; any custom font needs the same Canvas-bitmap approach RemoteViews has required all along) — but two other things made it worth adopting anyway, both verified before switching, not assumed:

- **License**: SIL OFL 1.1 (Google Fonts), free to use/modify/redistribute — no download cap, no registration, no non-transferability, unlike Dana Yad's free-tier commercial terms.
- **Latin coverage**: verified via `fontTools` cmap inspection — 27/27 Hebrew, 52/52 Latin, in both regular and bold weight, in the *same* font family. Dana Yad (like Refoyl and Solitreo before it) had zero Latin glyphs, which is what caused the "ChatGPT renders lighter than the Hebrew" bug above — English runs fell back to a differently-styled system font. That whole fallback mechanism (`withLatinCursiveFallback()`, `RunTypefaceSpan`) is now deleted; `TextBitmapHelper` loads the font's own real regular/bold files directly (`getTypeface(context, bold)`) and uses them for the whole string, headline and body alike.

Downloaded directly from Google's font-serving CDN (`fonts.gstatic.com`, via the `fonts.googleapis.com/css2` endpoint) rather than a packaged zip, so both weights were fetched and cmap-verified independently before being wired in. Verified live on-device (not just compiled): headline bold is now the font's genuine bold weight, not a synthesized fake-bold stroke; a real full-article fetch containing an embedded English name and email-style handle (`(ahuvaalperon@) Ahuva Alpron`) rendered in the identical weight and style as the surrounding Hebrew, confirming the fallback-mismatch bug is gone. Real-device screenshots (list view, individual headline rows, expanded article, full-article text, and a zoomed close-up of the mixed-script proof) were assembled into a "Feed Fidelity Check" verification artifact for review before committing to the switch.

`dana_yad.otf` and the earlier `refoyl.ttf`/`solitreo.ttf` trial fonts remain untracked in `app/src/main/assets/fonts/` and `app/src/main/res/font/` as historical leftovers from the font-selection process — none are referenced by any code path anymore.
