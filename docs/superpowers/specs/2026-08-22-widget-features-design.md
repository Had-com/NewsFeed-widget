...
**Open question, not resolved:** Miriam Libre Bold (the current Glamour headline font) is a clean bold sans-serif, not a handwriting/cursive style — the feature has never actually delivered on its "handwriting font" description. Searched Google Fonts' metadata for family Hebrew-subset + Handwriting-category and found no matches — genuine cursive/script-style Hebrew fonts appear to be scarce in that catalog compared to Latin script (a few exist via specialized foundries like AlefAlefAlef, but likely aren't Google-Fonts-licensed for free bundling the same way). Needs a product decision: source and vet a different (non-Google-Fonts) free/licensable Hebrew script font, or stop describing this as "handwriting" and rename the feature/theme copy to match what it actually is.
