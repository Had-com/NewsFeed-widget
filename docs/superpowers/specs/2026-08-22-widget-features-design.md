...
| 6 | New, found while re-verifying item 1: "Open article in: Share sheet" did nothing — logcat showed `ActivityNotFoundException: No Activity found to handle null`, with the launched PendingIntent's data replaced by an internal `glance-action:/` placeholder | `Intent.createChooser()`'s wrapper structure (action=CHOOSER, real target tucked into `EXTRA_INTENT`) doesn't survive `actionStartActivity()`'s handling, unlike a plain `Intent` — Browser mode (`ACTION_VIEW`) was unaffected. Dropped the `createChooser()` wrapper; Android still shows its own disambiguation picker automatically for a plain `ACTION_SEND` when more than one app can handle it. Not yet re-verified on-device as of this write-up. |

**Known remaining issue, not investigated:** the QA pass also noted article expand/collapse state appeared to reset intermittently between taps during testing, seemingly tied to periodic widget re-composition. Flagged for a future session, not chased here.
