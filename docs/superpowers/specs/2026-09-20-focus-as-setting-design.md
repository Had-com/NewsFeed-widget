...
**Goal.** One widget in the "Add widget" picker ("NewsFeed"). Focus (tap-to-enlarge) becomes a per-placed-widget setting, "When I tap an article: Expand in place / Focus (enlarge)", default Expand. The Focus-only control (Background rows size) is shown only when Focus is selected.

**Non-goals.** No change to Focus rendering (scale, tint, header N/M and −/+ buttons, memory budget). No alias or migration for already-placed Focus widgets: `NewsFeedFocusWidgetReceiver` is deleted, so those widgets disappear from the home screen on update. The user accepted this and will re-add "NewsFeed" and choose Focus. No new Gradle flavors, no CI change.
...
- `focusBackgroundScale` (default 0.5) is unchanged and simply ignored when tapMode is expand.
...
| `WidgetConfigActivity.kt` | Delete the receiver-class check (lines ~134-136) and the `NewsFeedFocusWidget().updateAll` call. `isFocusWidget` in the Composable becomes `config.tapMode == TapMode.FOCUS.key`, so the slider appears and disappears live as the dropdown changes. |
...
- **Memory budget.** `worstCaseRowScale` uses the derived flag, so a Focus-mode widget keeps its larger reserve and an Expand widget keeps 1×. Switching mode re-renders with the right budget. No new bitmap sizes are introduced.
...
- `docs/PRD.md`: lines 11 and 42 (section becomes "Focus Mode (tap-behavior setting)").
- `docs/DEBUG_PLAN.md`: section 7 retitled to the setting with the switch-live checks above; section 8 rewritten (one receiver, one clock alarm, orphan sweep checks); new subsection in the release log; the "both widget packages/types" item in section 6 reduced to one widget.
