...
| E-04 | P0 | Expanded-article length **Subtitle only** ⇒ ≤ 100 chars + `…`; **First paragraph** ⇒ ≤ 400; **Full article** ⇒ shows the description first; tap **Load full article ↓** ⇒ replaced by the fetched page body (two-phase); each mode matches the Settings preview. **Telegram sub-case (BUG-020):** Telegram behaves like any other feed **except** there is no Load full article button (the post text is already complete). A 1- or 2-line post shows its whole text as the headline and has no expand row; a 5-line post has a 2-line headline and expands to lines 3-5 only (headline not repeated); Subtitle ≤ 100 + `…`, First paragraph ≤ 400, Full shows the whole remainder; never shows "Download Context Embed View In Channel" page chrome | SS, DS (`full_article_*`) | none |
...
| N-15 | P1 | Update when **only** Focus widgets existed ⇒ the first refresh sweeps their leftovers and returns early (no widget to refresh); the periodic jobs are **not** cancelled by the worker (an empty list can be transient after boot/update), they stay armed and are only cancelled by `onDisabled` on a genuine last-widget removal; adding a widget refreshes normally | JS | none |
...
| N-31 | P1 | **Memory/size risk, long Telegram post (unverified / pending).** A ~4000-character Telegram post (many lines, description ≈ 4000 chars) in each theme (all 10) **and** in Focus mode, Full and First-paragraph length ⇒ the row expands with no "Can't show content" / RemoteViews failure, `adb logcat` has no `TransactionTooLarge`/OOM; Glamour shows one 1200-char chunk | SS, LC | none — **PENDING, not yet run on device** |
